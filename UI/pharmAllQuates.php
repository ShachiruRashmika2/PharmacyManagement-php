...
$stmt = $db->prepare("SELECT * FROM quotes WHERE pharmacyID = $pharmacy[id] ");
$result = $stmt->execute();
$quotes = [];
...
Alpha Pharma
  ...
  <nav class="absolute top-1/2 left-0 w-full -translate-y-1/2 flex flex-col items-center gap-4">
    <a href="pahrmacyDashboard.php" class="block px-6 py-2 text-slate-600  hover:bg-blue-100 text-blue-900 rounded-full">
      <i class="fa-solid fa-house text-2xl"></i>
    </a>
    <a href="pharmAllQuates.php" class="block px-6 py-2 bg-blue-100 text-blue-900 rounded-full">
 ...
 <a href="../PHP/logout.php" class="flex items-center shadow-xl justify-center px-6 py-2 text-slate-600 hover:bg-blue-100 hover:text-blue-900 rounded-full">
  ...
  <div class="flex-1 flex flex-col m-2 rounded-[36px]  glass  bg-[url('./Assets/Asset3.png')] bg-contain bg-center">
      ...
      <main class="p-6 overflow-y-auto">
        ...
        <h1 class="text-xl font-semibold ">Sent Qoutes</h1>
  ...
  <table class="min-w-full divide-y divide-gray-200 rounded-lg overflow-hidden shadow">
    ...
    <tbody class="bg-white divide-y divide-gray-100">
        <?php foreach ($quotes as $qoute): ?>
            ...
            $stmt = $db->prepare("SELECT name FROM users WHERE id =$qoute[customerID]");
            $result = $stmt->execute();
  ...
  <span class="text-gray-800 p-2 px-4 rounded-md flex items-center justify-center w-[50%] h-full <?php echo $bgClass; ?>">
    <?php echo $status; ?>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <a href="#" class="text-blue-600 hover:text-blue-900">Set</a>
                <a href="../PHP/deleteQuote.php?id=<?php echo $qoute['quoteID']; ?>" class="text-red-600 hover:text-red-900 ml-4">Delete</a>
            </td>
            </tr>
        <?php endforeach; ?>
